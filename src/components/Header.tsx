...
export default function Header() {
  return (
    <div class={tw`flex m-1 justify-between px-4 sm:px-5 bg-transparent font-mono`}>
      <nav
        class={tw
          `flex items-center justify-between w-full relative border-gray-200 dark:border-gray-700 mx-auto bg-opacity-60 sm:px-6 text-lg sm:text-xl align-bottom`}
      >
        <ul class={tw`space-x-4 sm:space-x-7 flex`}>
          {HEADER_ITEMS.map(({ name, path }) => {
            return (
              <a href={path} key={name} class={tw`hover:und`}>
                <li
                  class={tw
        ...
        <div class={tw`text-center sm:text-right sm:pr-5 w-full h-full align-middle sm:pb-2`}>
          <Toggle />
        </div>
      </nav>
    </div>
